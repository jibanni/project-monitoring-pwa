...
async function capacitorPhotoToFile(photo: { webPath?: string; format?: string }) {
  ...
  return new File([blob], `pms10-field-photo-${Date.now()}.${extension}`, {
    type: mimeType,
...
export default function ProjectUpdates() {
  ...
  const [nativePhotoLoading, setNativePhotoLoading] = useState(false)
  ...
  async function handleNativePhotoCapture() {
    ...
    try {
      ...
      appendPhotoFiles([file])
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error || '')
      const normalized = message.toLowerCase()

      if (normalized.includes('cancel') || normalized.includes('user cancelled')) {
        return
      }
      ...
      setErrorMessage(
        normalized.includes('permission')
          ...
          : message || 'Unable to open the camera/photo picker. Please try again.'
      )
    } finally {
  ...
  async function captureGps() {
    ...
    try {
      ...
      if (normalized.includes('permission') || normalized.includes('denied')) {
        ...
        setErrorMessage('GPS capture timed out. Move to an open area and try Update GPS again.')
      } else {
        setErrorMessage(
          message || 'Unable to capture GPS. Please verify that device Location is turned on and try again.'
        )
      }
    } finally {
      setGpsLoading(false)
        ...
        <form className="pu-form-card" onSubmit={handleSubmit}>
          ...
          <div className="pu-field-action-strip" aria-label="Field quick actions">
            ...
            {Capacitor.isNativePlatform() ? (
              <button
                type="button"
                className="pu-action-btn pu-action-photo"
                onClick={handleNativePhotoCapture}
                disabled={
                  saving ||
                  nativePhotoLoading ||
                  photoInputs.length >= MAX_PHOTOS_PER_UPDATE
                }
              >
                {nativePhotoLoading ? 'Opening Photos...' : 'Add / Take Photo'}
