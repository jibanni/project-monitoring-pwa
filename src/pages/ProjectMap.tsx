...
function buildMapProject(
  ...
  const newestUpdate = [...projectUpdates].sort(
    (a, b) => toTimestamp(b.created_at || b.inspection_date) - toTimestamp(a.created_at || a.inspection_date)
  )[0] || null
...
function FitMapToMarkers({
  ...
  useEffect(() => {
    ...
    if (validProjects.length === 1) {
      map.setView(
        [validProjects[0].displayLatitude as number, validProjects[0].displayLongitude as number],
        13
      )
      return
    }
...
export default function ProjectMap() {
  ...
  const filterOptions = useMemo(() => {
    ...
    projects.forEach((project) => {
      ...
      if (normalizeText(project.municipality)) municipalities.add(normalizeText(project.municipality))
      if (normalizeText(project.funding_source)) programs.add(normalizeText(project.funding_source))
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      ...
      const matchesStatus = statusFilter === 'All' || normalizeText(project.status) === statusFilter
      const matchesRisk = riskFilter === 'All' || normalizeText(project.risk_level) === riskFilter
  ...
  const summaryItems = useMemo<SummaryItem[]>(() => {
    ...
    const projectGps = filteredProjects.filter(
      (project) => project.coordinateSource === 'project'
    ).length

    const updateGps = filteredProjects.filter(
      (project) => project.coordinateSource === 'update'
    ).length

    const highRisk = filteredProjects.filter((project) =>
      normalizeText(project.risk_level).toLowerCase().includes('high')
    ).length
    ...
    return [
      ...
      {
        label: 'Project GPS',
        value: String(projectGps),
        helper: 'From create or edit project',
      },
      {
        label: 'Update GPS',
        value: String(updateGps),
        helper: 'From latest project update',
      },
      {
        label: 'High Risk',
      ...
      {
        label: 'Project Cost',
        value: formatPhpCompact(totalCost),
        helper: 'Filtered total cost',
      },
    ]
    ...
    <main className="pm-map-page">
      <section className="pm-map-hero">
        <div>
          ...
          <p>
            The map uses the latest valid coordinates entered from project creation, project
            editing, or project update records.
          </p>
        </div>

        <div className="pm-map-hero-actions">
          <button type="button" onClick={loadMapProjects}>
            Refresh Map
          </button>

          {canUpdateProject && (
            <Link to="/projects" className="pm-map-action-link">
              Project Register
            </Link>
          )}
        </div>
        ...
        <div className="pm-map-filter-grid">
          <label className="pm-map-search-field">
            ...
            <input
              ...
              placeholder="Search project, LGU, program, contractor..."
            />
          </label>
      ...
      <section className="pm-map-workspace">
        ...
        <aside className="pm-map-side-panel">
          ...
          <section className="pm-map-coordinate-card">
            <div className="pm-map-card-header">
              <div>
              ...
              <span>{coordinateIssueProjects.length}</span>
            </div>

            <div className="pm-map-issue-list">
              {coordinateIssueProjects.length === 0 ? (
                <div className="pm-map-empty">All filtered projects have usable coordinates.</div>
              ) : (
                coordinateIssueProjects.slice(0, 12).map((project) => (
                  <article className="pm-map-issue-card" key={project.id}>
                    <h3>{project.project_name || 'Untitled Project'}</h3>
                    <p>{getProjectLocation(project)}</p>
                    <span>{project.coordinateIssue}</span>

                    <div className="pm-map-project-actions">
                      <Link to={`/projects/${project.id}`}>View</Link>

                      {isAdmin && <Link to={`/projects/${project.id}/edit`}>Fix Coordinates</Link>}
                    </div>
                  </article>
                ))
              )}
