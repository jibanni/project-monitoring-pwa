...
for (const filePath of candidates) {
  ...
  const before = source.slice(Math.max(0, updateFormIndex - 3000), updateFormIndex)
  ...
  if (localHeroStart >= 0) {
    const absoluteHeroStart = Math.max(0, updateFormIndex - 3000) + localHeroStart
  ...
  if (source !== original) {
    backup(filePath, 'remove-project-update-hero')
...
if (modifiedTsxCount === 0) {
  console.error('Found candidate files but could not safely mark the Project Update hero.')
  process.exit(1)
}

backup(layoutCssPath, 'remove-project-update-hero')

removeCssBlock(layoutCssPath, 'PMS10 PROJECT UPDATE HERO TITLE CLAMP FIX')
removeCssBlock(layoutCssPath, 'PMS10 PROJECT UPDATE HERO SWIPEABLE TITLE')
removeCssBlock(layoutCssPath, 'PMS10 REMOVE PROJECT UPDATE HERO BANNER')

const cssBlock = `
/* =========================
   PMS10 REMOVE PROJECT UPDATE HERO BANNER
   Field mode: hides the large Project Update hero so the update form appears sooner.
========================= */
...
@media (max-width: 900px) {
  ...
  .project-update-page main,
  .project-updates-page main,
  .updates-page main {
    padding-top: 10px !important;
  }

  .project-update-page .page-content,
  .project-updates-page .page-content,
  .updates-page .page-content,
  .project-update-page .content-shell,
  .project-updates-page .content-shell,
  .updates-page .content-shell {
    padding-top: 10px !important;
  }
}
`
...
console.log('Project Update hero banner hidden for field mode.')
console.log('Previous Project Update hero title experiments removed from layout.css.')
